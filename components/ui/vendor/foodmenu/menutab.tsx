...
export function TabsMenu() {
  ...
  const fetchFoodItems = async () => {
    try {
      ...
      console.log('All food items',response.data);
    } catch (error) {
      const err = error as AxiosError<ErrorResponse>
  ...
  console.log("Food items before filter:", foodItems);
  const filteredItems = foodItems.filter((item) => {
    console.log("Comparing:", {
      itemCategory: item.menu_category,
      categoryId: category.id,
      matches: item.menu_category === category.id
    });
    return item.menu_category === category.id;
  });
